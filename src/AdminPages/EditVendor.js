...
// Define the categories to match your Mongoose enum
    ...
    const fetchVendor = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          // Set selected categories from database
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8">
              ...
              <div className="mb-8 flex items-center p-4 bg-white/60 backdrop-blur rounded-2xl border border-gray-200/50">
                ...
                <span className="ml-auto text-xs text-gray-500">
                  {formData.is_verified ? 'Visible to customers.' : 'Pending verification.'}
                </span>
              </div>

              {/* File Uploads - Restored original SVG icons and logic */}
              <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 mb-8">
                ...
                <div>
                  ...
                  {!bannerPreview ? (
                    <div className="relative border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-emerald-400 transition-colors group bg-white/40">
                      <input type="file" accept="image/*" ref={bannerInputRef} onChange={(e) => handleFileChange(e, 'banner')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                    ...
                    <div className="relative rounded-xl overflow-hidden border border-gray-200 shadow-sm group">
                      ...
                      <button type="button" onClick={() => handleRemoveImage('banner')} className="absolute top-3 right-3 bg-white/80 backdrop-blur-sm rounded-full p-2 text-red-500 shadow-sm hover:text-red-700 transition-all">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/40 to-transparent p-3 flex items-center justify-between">
                        {storeBanner ? <span className="text-xs font-medium text-white bg-emerald-500 px-2 py-0.5 rounded-full">New image</span> : removeBanner ? <span className="text-xs font-medium text-white bg-red-500 px-2 py-0.5 rounded-full">Will be removed</span> : null}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {!profilePreview ? (
                    <div className="relative border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-emerald-400 transition-colors group bg-white/40">
                      <input type="file" accept="image/*" ref={profileInputRef} onChange={(e) => handleFileChange(e, 'profile')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                    ...
                    <div className="flex flex-col items-center">
                      <div className="relative w-32 h-32 rounded-full border-2 border-gray-200 shadow-sm overflow-hidden group">
                        ...
                        <button type="button" onClick={() => handleRemoveImage('profile')} className="absolute top-1 right-1 bg-white/80 backdrop-blur-sm rounded-full p-1.5 text-red-500 shadow-sm transition-all hover:text-red-700">
                           <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                        </button>
                      </div>
