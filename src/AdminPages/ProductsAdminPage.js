...
const ProductsAdminPage = () => {
  ...
  // ── Filters live in a single state object ──────────────────────────────────
  const [filters, setFilters] = useState({
    search:'', category:'all', minPrice:'', maxPrice:'',
    isAvailable:'', sort:'name', page:1, limit:20,
  });

  // ── THE FIX: useEffect depends on the FULL filters object.
  //    fetchProducts receives the current filters snapshot as a parameter,
  //    so there is ZERO stale-closure risk — it always reads what was just set. ──
  const fetchProducts = useCallback(async (currentFilters) => {
    setLoading(true);
    try {
      // Build clean API params — strip empty/default values
      ...
      setStats({
        total,
        inStock:      rows.filter(p => p.countInStock > 0).length,
  ...
  // Fires every time ANY filter value (including page) changes
  useEffect(() => {
  ...
  const clearFilters = () => {
    setFilters({ search:'', category:'all', minPrice:'', maxPrice:'', isAvailable:'', sort:'name', page:1, limit:20 });
    setSelected(new Set());
  };
  ...
  const hasActiveFilters = filters.search || filters.category !== 'all'
    || filters.minPrice || filters.maxPrice || filters.isAvailable;
    ...
    <AdminLayout title="Products Management">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* Visibility — NO inline display allowed on these two elements */
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 16px 56px' }}>
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #F0F0F0', overflow:'hidden',
            ...
            {loading ? (
              ...
              <>
                {/* Desktop table — visibility via CSS class only, no inline display */}
                ...
                {/* Mobile cards — visibility via CSS class only, no inline display */}
